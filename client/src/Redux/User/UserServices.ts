...
export const resetPassword = async (
    ...
    try {
        const response = await axios.post(`${baseUrl}/users/resetpassword`, userData);
        if (response.status === 202) {
            successNotification(`${response.data.data.message}`);
        }
        return response.data;
...
export const updateUserInformation = async (data: UpdateUserProfile): Promise<AxiosPromise> => {
    try {
        const response = await axios.post(`${baseUrl}/users/updateuserinformation/${data.userId}`, data.userData);
        if (response.status === 202) {
